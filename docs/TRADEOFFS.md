...
| **LLM reranker** (SBERT retrieve top-20 → qwen3:8b reorders over full text) | **Unmeasured** — mechanism built + live-verified, leaderboard delta pending (1 LLM call/seed, deferred next session). Reorders *full* candidate text (no distillation, unlike LLM tags); ceiling is the SBERT base's recall@20 — it can only reorder what's retrieved | Query = base retrieval + 1 Ollama call (~seconds) on a cold cache; cached by `sha1(model+query+candidate-ids)` → warm reruns ~ms | Medium — yields a reordered list, no per-term reason (the "why this fits" rung is separate) | None (local Ollama, **no API key, zero new deps**) | Fine — reuses the base retriever; needs Ollama up only on a cold cache (else falls back to base order) | High (retrieve + LLM reorder + permutation reconcile + rerank cache) | A zero-shot reordering of a strong retriever's top-N when no domain cross-encoder exists — and one LLM call per query is affordable |
